...
\r Ferraris\lezione_10

*/

\\____DECIMA LEZIONE_________________________________________________
\\____________________________________________________________________
\\____________________________________________________________________

\\____________________________________________________________________
\\Programmi dati in classe LEZ 10
...
\\ ris=[];forprime(w=5,20,ris=concat(ris,w+1-ellap(ellinit([0,0,0,Mod(w-1,w),Mod(w-1,w)]),w)))
...
\\____________________________________________________________
\\ Programmi fatti per esercizio LEZ 10

\\____________________________________________________________
\\_______________________________________________________Es 1

\\ Lez 10 Es 1: ellisum
\\ Input: [a1,a2,a3,a4] parametri curva ellittica P=[xp,yp],Q=[xq,yq], punti già sulla curva ellittica
\\ Output: P+Q
\\ N.B. fare attenzione ai tipi...
{
ellisum(E,P,Q)=local(a1,a2,a3,a4,xp,yp,xq,yq,mP,lam,mu,xr,yr,R);
...
if(mP==Q,R=[0],
         ...
         mu=yp - lam*xp;
         xr=lam^2 + a1*lam - a2 - xp - xq;
         yr= -lam*xr - a1*xr - mu - a3;
         R=[xr,yr]
...
\\ Lez 10 Es 1bis : ellisumTEST
\\ Input: E=[a1,a2,a3,a4] parametri curva ellittica P=[xp,yp],Q=[xq,yq], punti del piano
\\ Output: P+Q se P e Q appartengono alla curva
{
ellisumTEST(E,P,Q)=local(a1,a2,a3,a4,r1,r2);
a1=E[1];a2=E[2];a3=E[3];a4=E[4];
r1=ellisoncurve(ellinit([a1,a2,a3,a4,0]),P);
r2=ellisoncurve(ellinit([a1,a2,a3,a4,0]),Q);
if(r1==1 && r2==1,return(ellisum(E,P,Q)),return("I punti non appartengono alla curva"));
}

\\______________________________________________________________
\\_________________________________________________________Es 2

\\ Lez 10 Es 2: ellipot, algoritmo potenza veloce sulle curve ellittiche
\\ Input: E curva ellittica, a punto di E, m potenza.
\\ Output: m*a nel gruppo delle curve ellittiche
{
...
\\ Lez 10 Es 2: ellipottest, algoritmo potenza veloce sulle curve ellittiche
\\ Input: E curva ellittica, a punto , m potenza.
\\ Output: m*a nel gruppo delle curve ellittiche, oppure "errore" se a non è in E
{
ellipotTEST(E,a,m)=local(u,t,v,test);
test=ellisoncurve(E,a);
if(test==1,return(ellipot(E,a,m)),return("a is not in E"));
}


\\______________________________________________________________
\\_________________________________________________________Es 3
...
if(mP==Q,R=[0],
         ...
         mu=Mod(yp - lam*xp,n);
         xr=Mod(lam^2 + a1*lam - a2 - xp - xq,n);
         yr=Mod( -lam*xr - a1*xr - mu - a3,n);
         R=[xr,yr]
...
xp=P[1];yp=P[2];xq=Q[1];yq=Q[2];
t1=Mod(xq-xp,n);
t2=Mod(2*yp + a1*xp + a3,n);
if(t1==Mod(0,n) && t2==Mod(0,n),return("input errati"),
                                tip= type(ellisumnNOTEST(E,P,Q,n));
                                if(tip=="t_VEC", return(ellisumnNOTEST(E,P,Q,n)),
                                                return(factor(n))
                                );
);
}
...
E=ellinit([Mod(0,n),Mod(0,n),Mod(0,n),Mod(a,n),Mod(b,n)]);
if(gcd(n,6)!=1,boo=0,if(gcd(4*a^3+27*b^2,n)!=1,boo=0,boo=1));
return([E,boo]);
}
...
\\ Lez  Es : ausiliaria, funzione pigreco
\\ Input: n intero.
\\ Output: cardinalità dell'insime dei numeri primi minori di  n.
{
pigreco(n)=local();
primepi(n);
}

\\ Lez  Es : ausiliaria: trova ai per l'algoritmo ECM
\\ Input: prim, B. B intero e prim primo minore di B
\\ Output: ai=max{ai t.c. prim^ai < B1}
{
trovai(prim,B)=local(a);
a=1;
while(prim^a < B,a = a+1);
return(a-1);
}

\\ Lez  Es : Algoritmo ECM
\\ Input: n composto non quadrato, B1 limite di calcolo (es 10000).
\\ Output: fatt fattore non banale di n.
/*
{
ecm(n,B1)=local(rv,g,E,P,ai,pivett,tip);

rv=randecm(n);
while(gcd(4*rv[3]^3+27*rv[4]^2,n)==n, rv=randecm(n));
g=gcd(4*rv[3]^3+27*rv[4]^2,n);
if(lift(g)>1,fatt=lift(g),E=ellilenstra(rv[3],rv[4],n)[1];P=[rv[1],rv[2]]);
...
forprime(pr=2,B,ex=trovai(pr,B);
...
\\________________________________________________________Es 5
...
\\ Output: R in {0,1}
{
minus2zerosMatr(M)=local(r,c,R);
r=matsize(M)[1];
c=matsize(M)[2];
R=M;
for(i=1,r,for(j=1,c,if(R[i,j]==-1,R[i,j]=0)));
R;
}
...
\\ Output: R in {0,1}
{
minus2zerosVett(V)=local(ris);
ris=2*vector(length(V),i,1)-V;
lift(Mod(ris,3))
}
...
\\ Output: R in {-1,+1}
{
zeros2minusMatr(M)=local(r,c,R);
r=matsize(M)[1];
c=matsize(M)[2];
R=M;
for(i=1,r,for(j=1,c,if(R[i,j]==0,R[i,j]=-1)));
R;
}
...
\\ Output: R in {-1,+1}
{
zeros2minusVett(V)=local(ris);
ris= 2*V-vector(length(V),i,1);
ris;
}
...
if(test==0, return("matrici incompatibili"),
    if(h==1 && k!=1 ,
      R[1,]=concat(A,A) ;
...
{
GRM(r,m)=local(cont,R);
if(r==0,R=vector(2^m,i,1));
if(r==m,R=matid(2^m));
if( r<m &&  r>0, R=RMprod(GRM(r,m-1),GRM(r-1,m-1)));
R;
}


\\ Lez 10 Es 5: hadecode(n,v)
\\ Input: n parametro, v parola da decodificare in {-1,+1}
\\ Output: decodifica della parola data.
\\N.B non c'è il test che rivela se ci sono più errori di quelli che si possono correggere
\\lo si puo' implementare: se pos è un vettore con più di un elemento, allora ERRORE.
{
hadecode(n,v)=local(H,Hm,sindrom,P1,P2,pos,ris);
H=hadamard(n);
Hm=-H;
sindrom=v*H;
