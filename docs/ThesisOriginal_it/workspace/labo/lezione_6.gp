...
\r Ferraris\lezione_6

*/

\\____SESTA LEZIONE_________________________________________________
\\____________________________________________________________________
\\____________________________________________________________________

\\____________________________________________________________________
\\Programmi dati in classe LEZ 6
...
\\vedere se è invertibile senza calcolare il det
{
matinvQ(a)=local(n);
n=matsize(a)[1];
...
\\ustao potenza veloce con le quadrature!
{
...
\\_____________________________________________
\\ Programmi fatti per esercizio LEZ 6

\\ Lez 6 Es 0: modulopoli
\\ Input: f(x) polinomio a coeff interi, p
\\ Output: f(x) a coeff modulo p ed f liftato
{
modulopoli(f,p)=local(q);
q=Mod(1,p)*f;
[q; lift(q)];
}

\\Lez 6 es 0: cinesep
\\input: riceve una lista di Mod(a(x),b(x)) e p primo
\\output: calcola il tcr modulando i coefficienti su p
{
cinesep(lista,p)=local(n,u,v);
n=length(lista);
u=chinese(Mod(1,p)*lista[1],lista[2]);
for(i=3,n,v=chinese(u,Mod(1,p)*lista[i]);u=v);
u;
}
...
\\        riceve inoltre n, p
\\ Output: restituisce la controimmagine di V tramite teta, con il TCR
{
gammaa(V,n,p) = local(M,s,vett);
M=lift(factormod(x^n-1,p));
s=matsize(M)[1];
vett=[];
for(i=1,s,vett = concat(vett, Mod(V[i],M[i,1]) ) );
risp=lift(cinesep(vett,p));
risp;
}
...
{
teta1(a,n,p)=local(f,s);
f=lift(factormod(x^n-1,p));
s=matsize(f)[1];
return(vector(s,i,Mod(a,f[i,1])))
}

\\E la sua inversa...

{
gamma1(V,n,p) = local(M,s,vett);
M=lift(factormod(x^n-1,p));
s=matsize(M)[1];
vett=[];
\\genero il vettore [...,Mod(V_i,M_i),...] ne faccio il cinese e il lift
for(i=1,s,vett = concat(vett, Mod(V[i],M[i,1]) ) );
risp=lift(chinese(vett));
risp;
}

\\Ma in questo modo gamma1 è l'inverso di teta1. Sono diversi da gammaa e teta
...
{
circolidem(n,p)=local(f,s,vett,matr);
f=lift(factormod(x^n-1,p));
s=matsize(f)[1];
vett=concat(1,vector(s-1,x,0));
circol(vett);
}
