...
\\tabellina azione di sgp su Z_r
\\input: V vettore che rappresenta gli elementi del sgp di Z_r^star, r
\\output: matrice delle moltiplicazioni.
{
tabellaAzione(V,r) = local(M);
   M = matrix(length(V),r,i,j,Mod(V[i]*(j-1),r));
return(lift(M));
}
